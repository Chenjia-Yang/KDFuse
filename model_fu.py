...
from FPB import Enhance_module,SIM
import argparse
import os
...
def parse_option():
    parser = argparse.ArgumentParser('argument for module_fusion')
    parser.add_argument('--h', type=int, default=128, help='height of images')
    parser.add_argument('--w', type=list, default=128, help='width of images')
    parser.add_argument('--channel', type=list, default=[16, 64, 128, 256], help='student channel')
...
class FusionNet(nn.Module):
    def __init__(self):
        ...
        self.sim1 = SIM(ch_seg[0], ch[2]//2)

        self.en_conv2 = Enhance_module(ch[3]//2, ch_seg[1])
        self.sim2 = SIM(ch_seg[1], ch[3]//2)
...
class FusionOnly(nn.Module):
    def __init__(self):
        super(FusionOnly, self).__init__()
        ch = [32, 64, 128, 256]
        ch_seg = [128, 320]
        self.conv1 = conv(2, ch[0])
        self.repblock1 = RepRFB(ch[0], ch[1])
...
class noSDIM(nn.Module):
    def __init__(self):
        ...
        # self.sim1 = SIM(ch_seg[0], ch[2])

        self.en_conv2 = Enhance_module(ch[3]//2, ch_seg[1])
        self.conv_en2 = conv(ch_seg[1], ch[3]//2)
        # self.sim2 = SIM(ch_seg[1], ch[3])
...
class FusionNet_twoVREM(nn.Module):
    def __init__(self):
        super(FusionNet_twoVREM, self).__init__()
        ch = [32, 128, 256]
        ch_seg = [128, 320]
        self.conv1 = conv(2, ch[0])
        self.repblock1 = RepRFB(ch[0], ch[1])
        ...
        self.en_conv1 = Enhance_module(ch[1]//2, ch_seg[0])

        self.sim1 = SIM(ch_seg[0], ch[1]//2)

        self.en_conv2 = Enhance_module(ch[2]//2, ch_seg[1])
        self.sim2 = SIM(ch_seg[1], ch[2]//2)

        self.rec_conv1 = conv(ch[2], ch[1])
        self.rec_conv2 = conv(ch[1], ch[0])
        self.rec_conv3 = nn.Sequential(
                nn.Conv2d(ch[0], 1, (3, 3), (1, 1), 1),
                nn.Tanh()
            )
    ...
    def forward(self, vi, ir):
        ...
        f1 = self.repblock1(f_shallow)
        f1_1, f1_2 = f1.chunk(2, dim=1)
        feat1 = self.en_conv1(f1_1)
        feat_list.append(feat1)
        f_sim1 = self.sim1(feat1, f1_1)
        f_deep1 = torch.cat((f_sim1, f1_2), dim=1)

        f2 = self.repblock2(f_deep1)
        f2_1, f2_2 = f2.chunk(2, dim=1)
        feat2 = self.en_conv2(f2_1)
        feat_list.append(feat2)
        f_sim2 = self.sim2(feat2, f2_1)
        f_deep2 = torch.cat((f_sim2, f2_2), dim=1)

        d1 = self.rec_conv1(f_deep2) + f_deep1
        d2 = self.rec_conv2(d1) + f_shallow
        output = self.rec_conv3(d2)
...
class FusionNet_fourVREM(nn.Module):
    def __init__(self):
        super(FusionNet_fourVREM, self).__init__()
        ch = [16, 32, 64, 128, 256]
        ch_seg = [128, 320]
        self.conv1 = conv(2, ch[0])
        self.repblock1 = RepRFB(ch[0], ch[1])
        ...
        self.repblock2 = RepRFB(ch[1], ch[2])
        self.repblock3 = RepRFB(ch[2], ch[3])
        self.repblock4 = RepRFB(ch[3], ch[4])
        ...
        self.rec_conv1 = conv(ch[4], ch[3])
        self.rec_conv2 = conv(ch[3], ch[2])
        self.rec_conv3 = conv(ch[2], ch[0])
        self.rec_conv4 = nn.Sequential(
                nn.Conv2d(ch[0], 1, (3, 3), (1, 1), 1),
                nn.Tanh()
            )
    ...
    def forward(self, vi, ir):
        ...
        f_4 = self.repblock4(f_deep2)

        d1 = self.rec_conv1(f_4) + f_deep2
        d2 = self.rec_conv2(d1) + f_deep1
        d3 = self.rec_conv3(d2) + f_shallow
